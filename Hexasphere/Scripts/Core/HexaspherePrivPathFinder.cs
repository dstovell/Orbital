				...
				public partial class Hexasphere : MonoBehaviour {
								...
								void ComputeRouteMatrix (int groupMask) {
												...
												for (int j = 0; j < tiles.Length; j++) {
																if (tiles [j].canCross && (tiles[j].group & groupMask) > 0) {	// set navigation bit
																				routeMatrix [j] = 1;
								...
								List<PFClosedNode> FindPathFast (int tileStartIndex, int tileEndIndex) {
												...
												while (open.tilesCount > 0) {
																...
																for (int i = 0; i < maxi; i++) {
																				...
																				int gridValue = routeMatrix [nextTileIndex] > 0 ? 1 : 0;
																				if (gridValue == 0)
																								continue;
																				...
																				int mNewG = pfCalc [currentTileIndex].g + gridValue;
																				...
																				case HeuristicFormula.SphericalDistance:
																								dist = (int)(Vector3.Angle (destinationCenter, tiles [nextTileIndex].center) * 1000f);
																								break;
																				case HeuristicFormula.Euclidean:
																								dist = (int)(Vector3.Distance (destinationCenter, tiles [nextTileIndex].center) * 1000f);
																								break;
																				case HeuristicFormula.EuclideanNoSQR:
																								dist = (int)(Vector3.SqrMagnitude (destinationCenter - tiles [nextTileIndex].center) * 1000f);
																								break;
																				}
																				pfCalc [nextTileIndex].f = mNewG + 2 * dist;
